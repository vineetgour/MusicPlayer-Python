...
class Toplevel1:
    def __init__(self, top=None):
        ...
        self.songName.configure(text='''Music!!!''')
    ...
    def setup_player(self):
        try:
            ...
            if self.my_player.get_db_status():
                ...
                raise Exception("Sorry! you can't save or load favourite!!!")
        ...
        self.Button10.configure(command=self.remove_song_from_favourite)
        self.playList.configure(font="Vivaldi 12")
        self.playList.bind("<Double-1>", self.list_double_click)
        img = tk.PhotoImage(file="./icons/play.png")
        self.top.iconphoto(self.top, img)
        self.top.title("Music-Dance to rhythm")
    ...
    def remove_song(self):
        ...
        try:
            if len(self.sel_index_tuple) == 0:
                raise NosongSelection("Please select a song to remove")
    ...
    def play_song(self):
        ...
        try:
            if len(self.sel_song_index_tuple) == 0:
                raise NosongSelection("Please select a song to play")

            self.song_name = self.playList.get(self.sel_song_index_tuple[0])
            self.show_song_details()
            if self.isThreadRunning is False:
    ...
    def load_previous_song(self):
        try:
            if not hasattr(self, "sel_song_index_tuple"):
                raise NosongSelection("Please select a song first")
    ...
    def add_song_to_favourites(self):
        songs = self.playList.curselection()
        try:
            if len(songs) == 0:
                raise NosongSelection("Please select a song to add favourites")
            song_name = self.playList.get(songs[0])
    ...
    def remove_song_from_favourite(self):
        songs_fav = self.playList.curselection()
        try:
            if len(songs_fav) == 0:
                raise NosongSelection("Please select a song first to remove")
            songs_fav = self.playList.get(songs_fav[0])
            result = self.my_player.remove_song_from_favourites(songs_fav)
            if "deleted" in result:
                self.playList.delete(songs_fav[0])
    ...
    def close_window(self):
        ...
        if result:
            ...
            messagebox.showinfo("Have a good day", "Thankyou")
            self.top.destroy()
